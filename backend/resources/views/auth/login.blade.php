...
<main class="flex-grow flex items-center justify-center p-6 min-h-screen bg-primary-50 dark:bg-gray-900 transition-colors duration-200">
    <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 animate__animated animate__zoomIn transition-colors duration-200">
        ...
        <form action="{{ url('login') }}" method="POST" class="space-y-5">
            ...
            <div class="flex items-center justify-between mt-2">
                <div class="flex items-center">
                    <input id="remember" name="remember" type="checkbox" class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded transition-colors duration-200">
                    <label for="remember" class="ml-2 block text-sm text-gray-700 dark:text-gray-300 transition-colors duration-200">
                        Remember me
                    </label>
                </div>
                <div class="text-sm">
                    <a href="{{ url('forgot-password') }}" class="font-medium text-primary-600 hover:text-primary-500 transition">
                        Forgot password?
                    </a>
                </div>
            </div>
        ...
        <div class="mt-8 text-center text-sm text-gray-600 dark:text-gray-400 transition-colors duration-200">
            ...
            <a href="{{ url('register') }}" class="font-bold text-primary-600 dark:text-primary-400 hover:text-primary-500 dark:hover:text-primary-300 transition-colors duration-200">
                Sign up now
            </a>
        </div>
    </div>
