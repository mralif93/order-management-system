...
@section('title', 'Customers Management | OMS Admin')
...
<div class="max-w-[1600px] mx-auto space-y-8 pb-12 transition-colors duration-300">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="animate__animated animate__fadeInLeft">
            <h1 class="text-3xl font-extrabold text-secondary-900 dark:text-white tracking-tight leading-none">Customers</h1>
            <p class="text-secondary-500 dark:text-slate-400 mt-2 font-medium">Manage your customer database and view their activity.</p>
        </div>
        <div class="animate__animated animate__fadeInRight">
            <button class="px-6 py-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 text-secondary-700 dark:text-slate-300 rounded-2xl font-black hover:bg-gray-50 dark:hover:bg-slate-800 transition flex items-center gap-2 shadow-sm">
                <i class="hgi-stroke hgi-download-04 text-xl"></i> Export Customers
            </button>
        </div>
    </div>
                    ...
                    <tr class="hover:bg-primary-50/20 dark:hover:bg-primary-900/5 transition-colors group">
                        <td class="py-5 px-6">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-2xl bg-primary-100 dark:bg-primary-900/30 text-primary-600 flex items-center justify-center font-bold text-sm shadow-sm">
                                    {{ substr($customer->name, 0, 1) }}
                                </div>
                                <div>
                                    <p class="font-bold text-secondary-900 dark:text-white leading-none">{{ $customer->name }}</p>
                                    <p class="text-[10px] text-secondary-400 dark:text-slate-500 mt-1 uppercase tracking-wider">#ID-{{ str_pad($customer->id, 5, '0', STR_PAD_LEFT) }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-5 px-6">
                            <div class="flex flex-col gap-1">
                                <span class="text-secondary-700 dark:text-slate-300 font-medium">{{ $customer->email }}</span>
                                <span class="text-xs text-secondary-400 dark:text-slate-500 font-bold tracking-tight">{{ $customer->phone ?? 'No Phone' }}</span>
                            </div>
                        </td>
                        <td class="py-5 px-6">
                            <div class="flex flex-col">
                                <span class="text-secondary-600 dark:text-slate-400">{{ $customer->city ?? 'N/A' }}, {{ $customer->state ?? 'N/A' }}</span>
                                <span class="text-[10px] text-secondary-400 dark:text-slate-500 uppercase tracking-widest">{{ $customer->country }}</span>
                            </div>
                        </td>
                        <td class="py-5 px-6">
                            <div class="flex flex-wrap gap-2">
                                @if($customer->is_locked)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400">
                                    Locked
                                </span>
                                @elseif($customer->is_active)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-100 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400">
                                    Active
                                </span>
                                @else
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-amber-100 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400">
                                    Inactive
                                </span>
                                @endif
                            </div>
                        ...
                        <td class="py-5 px-6">
                            <p class="text-secondary-600 dark:text-slate-400 font-medium">{{ $customer->created_at->format('M d, Y') }}</p>
                        ...
                        <td colspan="6" class="py-20 text-center">
                            <div class="flex flex-col items-center gap-3">
                                <div class="w-16 h-16 rounded-full bg-gray-50 dark:bg-slate-800 flex items-center justify-center text-gray-300 dark:text-slate-600">
                                    <i class="hgi-stroke hgi-user-multiple text-3xl"></i>
                                </div>
                                <p class="text-secondary-500 dark:text-slate-400 font-bold tracking-tight">No customers found.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
