    ...
    <div class="max-w-[1600px] mx-auto space-y-8 pb-12 transition-colors duration-300">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <button
                    class="px-4 py-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 text-secondary-700 dark:text-slate-300 rounded-xl text-xs font-bold hover:bg-gray-50 transition shadow-sm flex items-center gap-2">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 animate__animated animate__fadeInDown">
            <div>
                <div class="flex items-center gap-3 mb-2">
                    <span
                        class="px-3 py-1 bg-primary-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest">Order
                        Receipt</span>
                    <span
                        class="text-secondary-400 dark:text-slate-500 font-bold text-sm tracking-tight">{{ $order->created_at->format('M d, Y • h:i A') }}</span>
                </div>
                <h1 class="text-4xl font-black text-secondary-900 dark:text-white tracking-tight leading-none uppercase">
                    Order #{{ $order->order_number }}</h1>
            </div>

            <!-- Status Update Block -->
            <div
                class="bg-white dark:bg-slate-900 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 flex items-center gap-4">
                <div>
                    <p class="text-[10px] font-black text-secondary-400 dark:text-slate-500 uppercase tracking-widest mb-1">
                        Current Status</p>
                    @php
                        $statusColors = [
                    ...
                    @endphp
                    <p class="text-lg font-black {{ $statusColor }} uppercase tracking-tighter">{{ $order->order_status }}
                    </p>
                </div>
                ...
                <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="flex items-center gap-2">
                    ...
                    <select name="order_status"
                        class="bg-gray-50 dark:bg-slate-800 text-xs font-bold border-none rounded-xl focus:ring-2 focus:ring-primary-600/50 outline-none pr-8 py-2.5 dark:text-slate-200">
                    ...
                    <button type="submit"
                        class="p-2.5 bg-secondary-900 dark:bg-primary-600 text-white rounded-xl hover:shadow-lg transition active:scale-95">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 animate__animated animate__fadeInUp">
            ...
            <div class="lg:col-span-2 space-y-8">
                <div
                    ...
                    <div class="p-6 border-b border-gray-50 dark:border-slate-800">
                        <h2 class="text-xl font-black text-secondary-900 dark:text-white uppercase tracking-tighter">Ordered
                            Items</h2>
                    </div>
                                    ...
                                    <tr class="group">
                                        <td class="py-5 px-6">
                                            <div class="flex items-center gap-4">
                                                <div
                                                    class="w-12 h-12 rounded-xl bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-primary-600">
                                                ...
                                                <div>
                                                    ...
                                                    <p
                                                        class="text-xs text-secondary-400 dark:text-slate-500 mt-1 uppercase tracking-wider font-bold">
                ...
                @if($order->notes)
                    <div class="bg-amber-50 dark:bg-amber-900/10 rounded-3xl p-6 border border-amber-100 dark:border-amber-800">
                        ...
                        <p class="text-sm text-amber-800 dark:text-amber-300/80 font-medium leading-relaxed">{{ $order->notes }}
                        </p>
                    </div>
                @endif
            </div>
            ...
            <div class="space-y-8">
                <!-- Price Breakdown -->
                <div
                    class="bg-secondary-900 dark:bg-slate-900 rounded-3xl p-8 text-white shadow-xl border dark:border-slate-800">
                    <h3 class="text-lg font-black uppercase tracking-tighter mb-6 border-b border-white/10 pb-4">Payment
                        Summary</h3>
                    <div class="space-y-4">
                        <div class="flex justify-between text-secondary-400 font-bold text-xs uppercase tracking-widest">
                            <span>Subtotal</span>
                            <span class="text-white">{{ $order->currency }}
                        ...
                        <div class="pt-4 border-t border-white/10 flex justify-between items-end">
                            ...
                            <span class="text-3xl font-black text-primary-500">{{ $order->currency }}
                ...
                @php
                    ...
                    $custCity = $order->customer?->city ?? null;
                    $custState = $order->customer?->state ?? null;
                @endphp
                <div
                    class="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-slate-800">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xs font-black text-secondary-400 dark:text-slate-500 uppercase tracking-widest">
                            Customer Profile</h3>
                        @if($isGuest)
                            <span
                                class="px-2 py-1 bg-amber-100 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 text-[9px] font-black uppercase rounded-lg tracking-wider">
                                WhatsApp Guest
                            </span>
                        @endif
                    </div>
                    <div class="flex items-center gap-4 mb-6">
                        <div
                            class="w-14 h-14 rounded-2xl {{ $isGuest ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : 'bg-primary-100 dark:bg-primary-900/30 text-primary-600' }} flex items-center justify-center font-black text-2xl shadow-inner uppercase">
                            {{ strtoupper(substr($custName, 0, 1)) }}
                        </div>
                        <div>
                            <h4 class="text-lg font-black text-secondary-900 dark:text-white leading-none mb-1">
                                {{ $custName }}
                            </h4>
                            <p
                    ...
                    <div class="space-y-4">
                        @if($custEmail)
                            <div class="flex items-center gap-3 text-sm text-secondary-600 dark:text-slate-400 font-medium">
                                ...
                                <span class="truncate">{{ $custEmail }}</span>
                            </div>
                        @endif
                        @if($custPhone)
                            <div class="flex items-center gap-3 text-sm text-secondary-600 dark:text-slate-400 font-medium">
                        ...
                        @endif
                        @if($custCity || $custState)
                            <div
                                class="flex items-center gap-3 text-sm text-secondary-600 dark:text-slate-400 font-medium pb-2 border-b dark:border-slate-800">
                                <i class="hgi-stroke hgi-location-01 text-secondary-400 dark:text-slate-500 text-lg"></i>
                                <span>{{ implode(', ', array_filter([$custCity, $custState])) }}</span>
                            </div>
                        @endif
                        @if($order->source === 'whatsapp')
                            <div class="flex items-center gap-3 text-sm text-emerald-600 dark:text-emerald-400 font-bold">
                                <i class="hgi-stroke hgi-whatsapp text-lg"></i>
                                <span>Ordered via WhatsApp Shop</span>
                            </div>
                        @endif
                    </div>
                </div>
