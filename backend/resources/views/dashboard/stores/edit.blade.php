...
<div class="max-w-[1600px] mx-auto space-y-8 pb-12 transition-colors duration-300">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-secondary-400 dark:text-slate-500 text-sm font-medium mt-1">
                ...
                <a href="{{ route('public.store', $seller->store_slug) }}" target="_blank" class="text-primary-600 hover:underline">/shop/{{ $seller->store_slug }}</a>
            </p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.stores.update', $seller) }}" method="POST">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <!-- Left: Seller Summary -->
            <div class="space-y-6">
                <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 p-6 space-y-4">
                    <h2 class="font-black text-secondary-900 dark:text-white">Seller</h2>
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white font-black text-lg flex-shrink-0">
                            {{ strtoupper(substr($seller->name, 0, 2)) }}
                        </div>
                        <div>
                            <p class="font-black text-secondary-900 dark:text-white leading-none">{{ $seller->name }}</p>
                            <p class="text-[11px] text-secondary-400 dark:text-slate-500 mt-0.5">{{ $seller->email }}</p>
                        </div>
                    </div>
                ...
                <!-- Danger Zone -->
                <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-red-100 dark:border-red-900/30 p-6 space-y-4">
                    <h2 class="font-black text-red-600">Danger Zone</h2>
                    <p class="text-sm text-secondary-400 dark:text-slate-500">Deleting the store will remove the slug, name, and bio, and unfeature all products.</p>
