        ...
        <aside
            ...
            <div class="h-16 flex items-center px-6 border-b border-secondary-800 flex-shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-bold text-xl tracking-tight transition-opacity duration-300"
                        :class="{ 'opacity-100': sidebarOpen, 'opacity-0 lg:hidden': !sidebarOpen }">OMS Admin</span>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto sidebar-scroll py-6 px-3 space-y-1">
                ...
                <a href="{{ route('admin.dashboard') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ request()->routeIs('admin.dashboard') ? 'bg-primary-600 text-white' : 'hover:bg-secondary-800 text-secondary-400 hover:text-white' }}">
                ...
                <div class="pt-4 text-secondary-500 text-[10px] uppercase font-bold tracking-wider px-3 mb-2"
                    :class="{ 'block': sidebarOpen, 'hidden': !sidebarOpen }">Management</div>

                <a href="{{ route('admin.orders.index') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ request()->routeIs('admin.orders.*') ? 'bg-primary-600 text-white' : 'hover:bg-secondary-800 text-secondary-400 hover:text-white' }}">
                    <i class="hgi-stroke hgi-shopping-cart-01 text-xl"></i>
                    <span :class="{ 'block': sidebarOpen, 'hidden': !sidebarOpen }">Orders</span>
                </a>

                <a href="{{ route('admin.products.index') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ request()->routeIs('admin.products.*') ? 'bg-primary-600 text-white' : 'hover:bg-secondary-800 text-secondary-400 hover:text-white' }}">
                    <i class="hgi-stroke hgi-package text-xl"></i>
                    <span :class="{ 'block': sidebarOpen, 'hidden': !sidebarOpen }">Products</span>
                </a>

                <a href="{{ route('admin.customers.index') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ request()->routeIs('admin.customers.*') ? 'bg-primary-600 text-white' : 'hover:bg-secondary-800 text-secondary-400 hover:text-white' }}">
                    <i class="hgi-stroke hgi-user-multiple text-xl"></i>
                    <span :class="{ 'block': sidebarOpen, 'hidden': !sidebarOpen }">Customers</span>
                </a>

                <a href="{{ route('admin.sellers.index') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ request()->routeIs('admin.sellers.*') && !request()->routeIs('admin.stores.*') ? 'bg-primary-600 text-white' : 'hover:bg-secondary-800 text-secondary-400 hover:text-white' }}">
                    <i class="hgi-stroke hgi-user-account text-xl"></i>
                    <span :class="{ 'block': sidebarOpen, 'hidden': !sidebarOpen }">Sellers</span>
                </a>

                <a href="{{ route('admin.stores.index') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ request()->routeIs('admin.stores.*') ? 'bg-primary-600 text-white' : 'hover:bg-secondary-800 text-secondary-400 hover:text-white' }}">
                    <i class="hgi-stroke hgi-store-01 text-xl"></i>
                    <span :class="{ 'block': sidebarOpen, 'hidden': !sidebarOpen }">Stores</span>
                </a>

                <div class="pt-4 text-secondary-500 text-[10px] uppercase font-bold tracking-wider px-3 mb-2"
                    :class="{ 'block': sidebarOpen, 'hidden': !sidebarOpen }">System</div>

                <a href="#"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition hover:bg-secondary-800 text-secondary-400 hover:text-white">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header
                ...
                <div class="flex items-center gap-4">
                    ...
                    <!-- Search Box -->
                    <div
                        class="hidden md:flex items-center bg-gray-100 dark:bg-slate-800 rounded-lg px-3 py-1.5 w-64 lg:w-96">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative" x-data="{ profileOpen: false }" @click.away="profileOpen = false">
                        <button @click="profileOpen = !profileOpen"
                            ...
                            <div
                                class="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 flex items-center justify-center font-bold text-xs">
                                AD</div>
                        ...
                        <div x-show="profileOpen" x-cloak x-transition:enter="transition ease-out duration-100"
                            ...
                            <a href="{{ route('admin.profile') }}"
                                class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 font-medium {{ request()->routeIs('admin.profile') ? 'text-primary-600 dark:text-primary-400' : '' }}">
