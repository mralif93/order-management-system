    ...
    <div class="max-w-6xl mx-auto space-y-8 pb-12 transition-colors duration-300">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 animate__animated animate__fadeInDown">
            ...
            <div
                ...
                <form action="{{ route('seller.orders.update', $order) }}" method="POST" class="flex items-center gap-2">
                    @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 animate__animated animate__fadeInUp">
                                ...
                                @foreach($order->items as $item)
                                    <tr class="group">
                                        ...
                                        <td class="py-5 px-6 text-right font-medium text-secondary-500 dark:text-slate-400">
                                            {{ $order->currency ?? 'MYR' }} {{ number_format($item->unit_price, 2) }}
                                        </td>
                                        <td class="py-5 px-6 text-right font-black text-secondary-900 dark:text-white">
                                            {{ $order->currency ?? 'MYR' }} {{ number_format($item->total_price, 2) }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                @if($order->notes)
                    <div class="bg-amber-50 dark:bg-amber-900/10 rounded-3xl p-6 border border-amber-100 dark:border-amber-800">
                        <div class="flex items-center gap-3 mb-4 text-amber-700 dark:text-amber-400">
                            ...
                            <h3 class="text-sm font-black uppercase tracking-widest">Order Notes</h3>
                        </div>
            ...
            <div class="space-y-8">
                ...
                <div
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between text-secondary-400 font-bold text-xs uppercase tracking-widest">
                            <span>Subtotal</span>
                            <span class="text-white">{{ $order->currency ?? 'MYR' }}
                        ...
                        @if(isset($order->discount_amount) && $order->discount_amount > 0)
                            <div class="flex justify-between text-emerald-400 font-bold text-xs uppercase tracking-widest">
                                <span>Discount</span>
                                <span>- {{ $order->currency ?? 'MYR' }} {{ number_format($order->discount_amount, 2) }}</span>
                            </div>
                        @endif
                        <div class="pt-4 border-t border-white/10 flex justify-between items-end">
                            <span class="text-xs font-black uppercase tracking-[0.2em]">Total Amount</span>
                            <span class="text-3xl font-black text-primary-500">{{ $order->currency ?? 'MYR' }}
                ...
                <div
                    ...
                    <h3 class="text-xs font-black text-secondary-400 dark:text-slate-500 uppercase tracking-widest mb-6">
                        Customer</h3>
                    <div class="flex items-center gap-4 mb-6">
                        <div
                            class="w-14 h-14 rounded-2xl bg-primary-100 dark:bg-primary-900/30 text-primary-600 flex items-center justify-center font-black text-2xl shadow-inner uppercase">
                            {{ substr($order->customer->name ?? 'C', 0, 1) }}
                        </div>
                        <div>
                            <h4 class="text-lg font-black text-secondary-900 dark:text-white leading-none mb-1">
                                {{ $order->customer->name ?? 'N/A' }}</h4>
                            <p
                                class="text-[10px] text-primary-600 dark:text-primary-400 font-black uppercase tracking-widest">
                                Verified Customer</p>
                        </div>
                    </div>
                    <div class="space-y-4">
                        <div class="flex items-center gap-3 text-sm text-secondary-600 dark:text-slate-400 font-medium">
                            ...
                            <span class="truncate">{{ $order->customer->email ?? 'N/A' }}</span>
                        </div>
