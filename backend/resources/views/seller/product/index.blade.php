...
@section('title', 'My Products | OMS Seller')
    ...
    <div class="max-w-[1600px] mx-auto space-y-8 pb-12 transition-colors duration-300">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="animate__animated animate__fadeInLeft">
                <h1 class="text-3xl font-extrabold text-secondary-900 dark:text-white tracking-tight leading-none">My
                    Products</h1>
                <p class="text-secondary-500 dark:text-slate-400 mt-2 font-medium">Manage your store inventory, pricing, and
                    stock levels.</p>
            </div>
            <div class="animate__animated animate__fadeInRight">
                <a href="{{ route('seller.products.create') }}"
                    class="px-6 py-3 bg-primary-600 text-white rounded-2xl font-black hover:bg-primary-700 hover:shadow-lg hover:shadow-primary-600/20 transition-all flex items-center gap-2 shadow-md">
        ...
        @if(session('success'))
            <div
                ...
                <i class="hgi-stroke hgi-tick-double-01 text-xl"></i>
        ...
        <!-- Filters & Table Card -->
        <div
            class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden flex flex-col animate__animated animate__fadeInUp">
            <!-- Search Bar -->
            <div
                class="p-6 border-b border-gray-50 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <form action="{{ route('seller.products.index') }}" method="GET" class="relative group w-full sm:w-96">
                    <i
                            ...
                            <tr class="hover:bg-primary-50/20 dark:hover:bg-primary-900/5 transition-colors group">
                                <td class="py-5 px-6">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-secondary-400 dark:text-slate-500 mt-0.5">
                                                {{ Str::limit($product->description, 35) ?: 'No description' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-5 px-6">
                                    <span
                                        class="px-2 py-1 bg-gray-100 dark:bg-slate-800 text-secondary-600 dark:text-slate-400 rounded-lg text-xs font-bold uppercase tracking-wider">{{ $product->sku }}</span>
                                </td>
                                ...
                                <td class="py-5 px-6">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-1.5 h-1.5 rounded-full {{ $product->quantity > 10 ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse' }}">
                                        </div>
                                        <span
                                            class="font-bold text-secondary-700 dark:text-slate-300">{{ $product->quantity }}
                                            units</span>
                                    </div>
                                </td>
                                <td class="py-5 px-6">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400">Inactive</span>
                                    @endif
                                </td>
                            ...
                            <tr>
                                <td colspan="6" class="py-20 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-secondary-500 dark:text-slate-400 font-bold tracking-tight">No products
                                            found.</p>
                                        <a href="{{ route('seller.products.create') }}"
                                            class="text-sm font-bold text-primary-600 uppercase tracking-widest mt-2 hover:underline">Add
